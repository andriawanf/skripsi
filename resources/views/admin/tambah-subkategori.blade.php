        ...
        <div class="font-poppins ">
            <div class="flex justify-between items-center w-full mb-3 text-[10px]">
                ...
                <button
                    ...
                    data-modal-target="addSubkategori" data-modal-show="addSubkategori"><i class='bx bx-plus'
                        class="text-xs md:text-lg"></i><span>Tambah Sub-kategori</span></button>
                <button
                            ...
                            @foreach ($subkategori as $item)
                                <tr class="bg-white hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">
                                        <!-- Modal toggle -->
                                        <button type="button"
                                            data-modal-target="editSubkategoriModal-{{ $item->id }}"
                                            data-modal-show="editSubkategoriModal-{{ $item->id }}"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit
                                        </button>
                                        <!-- Edit subkategori modal -->
                                        <div id="editSubkategoriModal-{{ $item->id }}" tabindex="-1" aria-hidden="true"
                                            class="fixed top-0 left-0 right-0 z-50 items-center justify-center hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full backdrop-blur-sm">
                                            <div class="relative w-full max-w-2xl bg-white shadow-lg rounded-xl">
                                                <!-- Modal content -->
                                                <form action="{{ route('update-data-subkategori', $item->id) }}" method="POST"
                                                    enctype="multipart/form-data" class="relative">
                                                    @csrf
                                                    <!-- Modal header -->
                                                    <div
                                                        ...
                                                        <button type="button"
                                                            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                            data-modal-hide="editSubkategoriModal-{{ $item->id }}">
                                                        ...
                                                        <div class="grid grid-cols-6 gap-6">
                                                            ...
                                                            <div class="col-span-6 sm:col-span-6">
                                                                <label for="subkategori"
                                                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nam
                                                                    Sub-Kategori</label>
                                                                <input type="text" name="nama_subkategoris"
                                                                    id="nama_subkategoris"
                                                                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                                    value="{{$item->nama_subkategoris}}" required>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <!-- Modal footer -->
