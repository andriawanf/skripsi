<div
    ...
    @if (auth()->user()->foto == 'images/logo.png')
        <img src="{{ url('/' . auth()->user()->foto) }}" alt="foto profile"
            class="w-24 h-24 rounded-xl object-cover border-2 border-[#1e1e1e] z-10">
    @else
        <img src="{{ asset('storage/foto-profil/' . auth()->user()->foto) }}" alt="foto profile"
            class="w-24 h-24 rounded-xl object-cover border-2 border-[#1e1e1e] z-10">
    @endif
    <h1 class="z-10 text-xl font-medium capitalize">{{ auth()->user()->name }}</h1>
    <span class="z-10 text-sm font-normal">{{ auth()->user()->jabatan }} <span>- NIP:
            {{ auth()->user()->nip }}</span></span>
    ...
    <svg class="absolute bottom-0 left-0 z-0 mb-0" viewBox="0 0 375 283" fill="none"
